...
const forcaSenha = computed(() => {
  ...
  if (senha.value.length >= 6) pontos++;
  if (/[A-Z]/.test(senha.value)) pontos++;
  if (/[0-9]/.test(senha.value)) pontos++;
  if (/[^A-Za-z0-9]/.test(senha.value)) pontos++;

  return pontos;
});

const textoForcaSenha = computed(() => {
  if (forcaSenha.value <= 1) return "FRACA";
  if (forcaSenha.value === 2) return "MÉDIA";
  if (forcaSenha.value === 3) return "BOA";
  return "FORTE";
});
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (typeof data === "object") {
        erro.value = Object.values(data)
          .flat()
          .join(" ");
      } else {
        erro.value = "Erro ao criar sua conta.";
      }
    }
  } catch (err) {
  ...
  <div class="auth-page">
    ...
    <main class="auth-content">
      <section class="editorial">
        ...
        <p>
          Crie seu espaço na Padaria Martins
          e deixe seus próximos pedidos ainda
          mais simples.
        </p>
        ...
        <div class="visual">
          <div class="image-frame">
            ...
            <span class="image-label">DESDE 1987</span>
          </div>
